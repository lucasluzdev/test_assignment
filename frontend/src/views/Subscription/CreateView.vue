<template>
  <section class="container-fluid bg-emerald-50">
    <div class="container flex flex-col justify-center items-center h-screen">
      <h2 class="text-4xl text-center font-bold bg-emerald-600 p-2 w-full">
  ...
  methods: {
    createSubscription() {
      ...
      if (isValid.error) {
        this.$swal.fire({
          title: "Warning!",
          description: "Check the information provided.",
          icon: "warning",
        ...
        create(isValid.data).then((response) => {
          ...
          if (data.status === 201) {
            ...
            this.$swal.fire({
              title: "Error!",
              description:
                "Error while trying to store new subscriber. Try again later!",
              icon: "error",
              showConfirmButton: false,
              timer: 1200,
            });
